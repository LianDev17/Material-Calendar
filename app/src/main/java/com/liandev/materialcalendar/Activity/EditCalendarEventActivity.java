...
import com.liandev.materialcalendar.databinding.EditCalendarEventActivityBinding;
...
public class EditCalendarEventActivity extends AppCompatActivity {
    private EditCalendarEventActivityBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) { 
        ...
        binding = EditCalendarEventActivityBinding.inflate(getLayoutInflater());
        ...
        binding.appbar.setTitle(eventData.getName());              
        binding.editEventTitle.setText(eventData.getName());     
        binding.editEventDescription.setText(eventData.getDescription());     
       ...
       binding.editEventButton.setOnClickListener(new View.OnClickListener() {
           @Override
            public void onClick(View v) {   
              if(TextUtils.isEmpty(binding.editEventTitle.getText().toString()) == true){
                 Toast.makeText(getApplicationContext(), "Donnez un nom à votre événement avant de la modifier.", Toast.LENGTH_LONG).show();  
              } else if(startDate.getTime() > endDate.getTime()){
              Toast.makeText(getApplicationContext(), "La date de fin doit être après celle de début.", Toast.LENGTH_LONG).show();  
              } else {
                 googleCalendarHelper.updateEvent("primary", eventData.getId(), binding.editEventTitle.getText().toString(), binding.editEventDescription.getText().toString(), startDate.getTime(), endDate.getTime());
                 Intent i = new Intent();
       ...
       binding.deleteEventButton.setOnClickListener(new View.OnClickListener() {
           @Override
            public void onClick(View v) {  
               googleCalendarHelper.deleteEvent("primary", eventData.getId()); 
               Intent i = new Intent();
               i.putExtra("TYPE","CALENDAR");       
               setResult(Activity.RESULT_OK, i);             
               finish();     
